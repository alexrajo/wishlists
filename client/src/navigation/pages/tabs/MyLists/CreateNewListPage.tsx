import { View, Center, Heading, FormControl, Input, TextArea, ScrollView, Radio, Flex , Box, Button, Modal, Text, VStack, Pressable } from "native-base";
            ...
            <Center p={5}>
                ...
                <FormControl>
                    ...
                    <Flex flexDirection="row">
                        ...
                        <VStack flex={1} bg="white" marginLeft={2} rounded="md" borderColor="gray.300" borderWidth={1}>
                            <Button flex={1} m={2} onPress={() => setShowModal(true)}>+</Button>
                            <Button flex={1} marginX={2} marginBottom={2} isDisabled={selectedItemIndex === undefined} bg="red.500" onPress={onDeleteItemPressed}>-</Button>
                        </VStack>
                    </Flex>
                    <Button m={5} isLoading={isPending} onPress={onCreateWishlistPressed}>CREATE WISHLIST</Button>
                    <CreateItemModal showModal={showModal} setShowModal={setShowModal} onConfirm={onModalConfirmed}/>
                </FormControl>
            </Center>
        </View>
    );
...
const CreateItemModal = ({showModal, setShowModal, onConfirm}: {showModal: boolean, setShowModal: (arg0: boolean) => void, onConfirm?: (arg0: string) => void}) => {
    ...
    <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
        <Modal.Content maxWidth="80%">
            ...
            <Modal.Body>
                <VStack>
                    <Text>Text</Text>
                    <Input isRequired type="text" placeholder="Item name and description" value={text} onChangeText={setText} clearButtonMode="always"/>
                </VStack>
            </Modal.Body>
            <Modal.Footer>
...
const ItemsListItem = ({index, itemText, selectedIndex, onPress}: {index: number, itemText: string, selectedIndex?: number, onPress?: React.Dispatch<React.SetStateAction<number | undefined>>}) => {
    return (
        <Pressable key={index} onPress={() => {
            ...
            <Box flex={1} p={2} bg={selectedIndex !== undefined && selectedIndex === index ? "gray.400" : (index%2 === 0 ? "gray.50" : "gray.200")}>
                <Text fontWeight={selectedIndex !== undefined && selectedIndex === index ? "semibold" : "normal"}>{itemText}</Text>
            </Box>
        </Pressable>
    );
}
